...
public class GetLatestCurrencyExchangeRateByShortNameQueryHandler : BaseQueryHandler<GetLatestCurrencyExchangeRateByShortNameQuery, CurrencyExchangeRate?>
{
    ...
    public override async Task<DataResult<CurrencyExchangeRate?>> ExecuteAsync(
        GetLatestCurrencyExchangeRateByShortNameQuery request, CancellationToken cancellationToken)
    {
        var quoteCurrency = await currencyRepository.GetByAsync("ShortName", request.QuoteCurrencyShortName, cancellationToken);
        if (quoteCurrency == null)
            return DataResult<CurrencyExchangeRate?>.Failure($"Currency with short name '{request.QuoteCurrencyShortName}' not found.");
