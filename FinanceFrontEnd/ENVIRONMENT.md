...
-   `API_URL` — full HTTPS URL to the backend API (e.g. https://api.example.com). This is required in production and validated at startup/CI.

Optional override (use only in controlled staging/test)

-   `ALLOW_INSECURE_API=true` (server/CI) — allows `http://` API URLs for environments where HTTPS is intentionally not used. Use sparingly and never in public production.
...
-   The repo contains a GitHub Actions workflow `.github/workflows/validate-config.yml` that runs `scripts/validate-config.js`. The script:
    -   Fails if `API_URL` is missing or uses non-HTTPS in production unless ALLOW_INSECURE_API is set.
    -   Fails if Dockerfiles contain secret-like ARGs.

Example: provide `API_URL` and optional `ALLOW_INSECURE_API` via GitHub Actions secrets

1. In the repository settings -> Secrets, create `API_URL` (the HTTPS API URL) and, if needed for staging, `ALLOW_INSECURE_API`.
...
-   Add environment-level secrets (for example `API_URL`, `AUTH0_CLIENT_SECRET`) to the `staging` environment.
