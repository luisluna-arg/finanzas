...
const FetchTable: React.FC<FetchTableProps> = ({
  ...
  // Hide component if hideIfEmpty is true and there's no data
  if (hideIfEmpty && (!data || data.length === 0)) {
    return null;
  }
  ...
  const getColumnValue = (columnSettings: unknown, record: Row) => {
    ...
    if (mapper) {
      ...
      if (Object.hasOwn(mapper, 'label')) {
        ...
        return label ? (record as Record<string, unknown>)[String(label)] : undefined;
      }
    }
    ...
    return (
      <TableRow>
        {columns.map((col, index) => {
          ...
          if (column.formatter)
            displayValue = (column.formatter as (v: unknown) => unknown)(displayValue);

          return (
            <TableCell className={column.class} key={`${name}-${column.id}-${index}`}>
              <span className={cssClasses}>{displayValue == null ? '' : String(displayValue)}</span>
            </TableCell>
          );
        })}
      </TableRow>
  ...
  const TableContent = () => (
    <Table>
      ...
      {showTotals && data && data.length > 0 && columns.some((c) => (c as Column).totals) && (
        <TableFooter>
          <TableRow key={`${name}-totals-row`}>
            {columns.map((col, index) => {
              ...
              const value = (data ?? []).reduce(
                (acc, item) =>
                  (column.totals!.reducer as (a: unknown, r: Row) => unknown)(acc as unknown, item),
                0 as unknown
              );

              let cssClasses = '';
              if (column.conditionalClass) {
                if (Array.isArray(column.conditionalClass)) {
                  const useConditional = column.conditionalClass.some((c) => c.eval(value));
                  cssClasses = useConditional ? column.conditionalClass[0].class : '';
                } else {
                  cssClasses = column.conditionalClass.eval(value)
              ...
              if (column?.totals?.formatter && typeof column.totals.formatter === 'function') {
                formattedValue = column.totals.formatter(value);
              }
              ...
              return (
                <TableCell
                  ...
                  key={cellKey}
                >
                  <span className={cssClasses}>{String(formattedValue)}</span>
                </TableCell>
              );
            })}
          </TableRow>
