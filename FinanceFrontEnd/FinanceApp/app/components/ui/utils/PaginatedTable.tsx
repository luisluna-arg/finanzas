...
import ActionButton, { ACTION_BUTTON_KIND, VARIANTS } from '@/components/ui/utils/ActionButton';
...
const PaginatedTable: React.FC<PaginatedTableProps> = ({
  ...
  const getSelectIds = (isSelected?: boolean) => getSelectCheckboxes(isSelected).map((i) => i.id);

  const validateForm = (inputs?: Array<HTMLInputElement | HTMLSelectElement>) => {
    const resolved = inputs ?? getEditRowInputs();
    if (!resolved) return;
    for (const input of resolved) {
      if (!input.classList.contains('invisible') && input.type !== 'checkbox' && !input.value) {
        break;
      }
    }
  };
  ...
  const ColumnValue = ({ columnSettings, record }: { columnSettings: Column; record: Row }) => {
    ...
    if (columnSettings.type === InputType.Boolean) {
      return <Checkbox id={String(record.id)} checked={Boolean(columnValue)} />;
    }

    return <>{columnValue}</>;
  };

  const getEditRowDefaultValue = (columnEditable?: { defaultValue?: unknown }) => {
    if (columnEditable?.defaultValue === undefined || columnEditable?.defaultValue === null)
      return '';
  ...
  const ActionsRow = ({ header = false }: ActionsRowProps) => {
    const AddButton = () =>
      adminAddEnabled && (
        <ActionButton type={ACTION_BUTTON_KIND.add} className={['me-2']} onClick={onAddAction} />
      );
              ...
              return (
                ...
                >
                  <Input
                    ...
                    settings={column as unknown as InputControlSettings}
                  />
                </TableCell>
              );
            } else {
              return (
                <TableCell className={'centered'} key={`${name}-${columnId}-${index}`}></TableCell>
              );
            }
          })}
  ...
  const LoadingTableBody = ({
    ...
    return (
      <TableBody>
        <TableRow>
          <TableCell colSpan={columnsLength + (adminEnabled ? 1 : 0) + (adminAddEnabled ? 1 : 0)}>
            <center>
              <LoadingSpinner />
            </center>
          </TableCell>
