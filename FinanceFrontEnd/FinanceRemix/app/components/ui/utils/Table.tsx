...
function resolveClassName(...classNames: (string | string[] | undefined)[]): string {
  const flatClasses: string[] = [];
  
  for (const className of classNames) {
    if (Array.isArray(className)) {
      flatClasses.push(...className.filter(Boolean));
    } else if (className) {
      flatClasses.push(className);
    }
  }
  ...
  return (
    <TableUI>
      <TableHeader>
        <TableRow>
          {columns.map((c) => (
            <TableHead key={`${c.id}-head`} className={resolveClassName(c.headerClassName, c.className)}>
              {c.label}
            </TableHead>
          ))}
        </TableRow>
      ...
      <TableBody>
        {data.map((d: unknown, idx: number) => {
          const key =
            d && typeof d === 'object' && 'id' in (d as Record<string, unknown>)
              ? String((d as Record<string, unknown>).id)
              : idx.toString();
          return (
            <TableRow key={key}>
              {columns.map((c: TableColumn) => (
                <TableCell key={`${c.id}-body`} className={resolveClassName(c.className)}>
